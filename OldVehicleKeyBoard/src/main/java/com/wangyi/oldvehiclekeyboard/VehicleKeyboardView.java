...
public class VehicleKeyboardView extends KeyboardView {
    ...
    private void init(Context context) {
        Log.e("VehicleKeyboardView", "context = " + context);
        ...
        setPreviewEnabled(true);// 无法再popupwindow上展示popupwindow，而预览恰恰就是一个popupwindow
        ...
        if (keys != null && keys.size() > 0) {
            ...
            for (Keyboard.Key key : keys) {
                ...
                if (Arrays.asList(
                        ...
                        73, 79).contains(key.codes[0])
                ) {
                    dr = getContext().getDrawable(R.drawable.btn_gray);
//                    dr = ResourcesCompat.getDrawable(getContext().getResources(), R.drawable.btn_gray, null);
                } else {
                    dr = getContext().getDrawable(R.drawable.btn_white);
//                    dr = ResourcesCompat.getDrawable(getContext().getResources(), R.drawable.btn_white, null);
                }
                ...
                // 2 绘制文字
                if (key.label != null) {
                    if (Arrays.asList("ABC".hashCode(), -2).contains(key.codes[0])) {
